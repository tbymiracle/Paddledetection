...
class RFP(FPN):
    ...
    def __init__(self,
        ...
        self.rfp_modules = nn.LayerList()
        print(rfp_backbone)
        for rfp_idx in range(1, rfp_steps):
            rfp_module = create(rfp_backbone['type'])
            self.rfp_modules.append(rfp_module)
        self.rfp_aspp = ASPP(self.out_channel, aspp_out_channels,
                             aspp_dilations)
    ...
    def forward(self, inputs):
        ...
        for rfp_idx in range(self.rfp_steps - 1):
            rfp_feats = [x[0]] + list(self.rfp_aspp(x[i])
            ...
            for ft_idx in range(len(x_idx)):
                add_weight = F.sigmoid(self.rfp_weight(x_idx[ft_idx]))
